...
document.addEventListener("DOMContentLoaded", function () {
    ...
    sendBtn.addEventListener("click", async function () {
        ...
        if (isQuestion && activeVideo) {
            appendMessage("Bot", `Thinking about your question regarding "${activeVideo.title}"...`);
            setTimeout(() => {
                appendMessage("Bot", `Based on "${activeVideo.title}", here is a clarification: The core concepts discussed provide a great framework for understanding this topic. If you need clarity on a specific timestamp, let me know!`);
            }, 1000);
            return;
        }

        if (isQuestion && !activeVideo) {
            appendMessage("Bot", `It looks like you're asking a question, but there's no active video right now. Please tell me a topic to search for first!`);
            return;
        }
    ...
    function appendVideo(videoId, videoTitle) {
        ...
        videoElement.innerHTML = `
            <iframe width="100%" height="315" src="https://www.youtube.com/embed/${videoId}?autoplay=1" frameborder="0" allowfullscreen></iframe>
            <br>
        ...
        summarizeBtn.addEventListener("click", () => {
            appendMessage("Bot", `Generating summary for "${videoTitle}"...`);
            setTimeout(() => {
                const summary = `Here is a quick AI summary of "${videoTitle}": This video covers the essential key points and provides an in-depth explanation of the topic. It's highly recommended if you're looking to understand the fundamentals and advanced insights!`;
